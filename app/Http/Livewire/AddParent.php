...
class AddParent extends Component
{
    ...
    public function updated($propertyName)
    {
        $this->validateOnly($propertyName , [
            ...
            'Phone_Father' => 'required|regex:/[XI0-9\/]+/|unique:my_parents|min:10|max:20',
            'National_ID_Mother' => 'required|string|min:10|max:10|regex:/[0-9]{9}/',
            'Passport_ID_Mother' => 'min:10|max:10',
            'Phone_Mother' => 'required|regex:/[XI0-9\/]+/|unique:my_parents|min:10|max:20'
        ]);
    }
        ...
        try {
            ...
            if(!empty($this->photos)){
                foreach ($this->photos as $photo){
                    ...
                    //$photo->getClientOriginalName()  == name image
                    //$disk = 'parent_attachments' => config->filesystem
                    //storage uploade file//
                    $photo->storeAs($this->National_ID_Father , $photo->getClientOriginalName() ,$disk = 'parent_attachments');
                    ParentAttachment::create([
                        'file_name' => $photo->getClientOriginalName() ,
                        //last request insert db -> id
            ...
            $this->clearForm(); // clear All Input
            $this->currentStep = 1;
        } catch (\Exception $e) {
